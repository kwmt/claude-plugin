...
- **ビルド番号重複** ("Build number already exists") → A) `build_number` オプションで明示指定 B) `latest_testflight_build_number + 1` の値を確認して再実行
- **TestFlight アップロード失敗** → A) API Key の環境変数 3 つ (`APP_STORE_CONNECT_API_KEY_KEY_ID`, `ISSUER_ID`, `KEY`) を確認 B) 手動アップロード手順を案内
- **タグ作成失敗** → A) `git tag -l` で既存タグとの重複を確認 B) `git fetch --tags` でリモートと同期
